...
InsertTList( Zygote * zyg, TList * first, double time, int op ) {
    ...
    TList *new_;                 /* used to allocate memory for new element */
    ...
    do {
        ...
        if( fabs( time - current->time ) < BIG_EPSILON ) {
            ...
            if( current->n == n ) {     /* if new time really close to an existing time point and if no cell division happened... */
                if( current->op == MITOTATE ) { /* if we are skipping epsilon after MITOSIS start */
                    new_ = ( TList * ) malloc( sizeof( TList ) );
                    new_->time = time;   /* -> allocate new_ element for right after */
                    new_->n = n; /* cell division has occurred */
                    new_->op = op;
                    new_->next = current->next;
                    current->next = new_;
                    return first;
                }
                current->op |= op;      /* -> just add new_ op to existing one - from the resulting number we can obtain */
        ...
        } else if( time > current->time ) {     /* new_ time > than current time */
            if( fabs( time - current->next->time ) < BIG_EPSILON ) {    /* is it */
                ...
                new_ = ( TList * ) malloc( sizeof( TList ) );    /* the next time point */
                new_->time = time;       /* -> allocate new_ element */
                new_->n = n;
                new_->op = op;
                new_->next = current->next;
                current->next = new_;
