...
#ifdef OPENMP
#pragma omp threadprivate(f,ssACT,ssGLC,S)
#endif
...
int amigoRHS_B3(realtype t, N_Vector y, N_Vector ydot, void *data){
    ...
    *(1+x(axGLX)/p(apgaceBAKKG)))/p(apgaceBAKL)
    ...
    //S*f'
    long int l_1 = (long int) l;
    long int n_1 = (long int) n;
    long int m_1 = (long int) m;
    long int lda_1 = (long int) lda;
    long int ldb_1 = (long int) ldb;
    long int ldc_1 = (long int) ldc_1;

    dgemm_(&transa, &transb, &l_1, &n_1, &m_1, &alpha, S, &lda_1, f, &ldb_1, &beta, xdot, &ldc_1);
