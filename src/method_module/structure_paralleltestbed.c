...
void setoutputmatlabres_(void *omt_, double *r, int *sizer){
     outputmatlab *omt;
     int i;

     omt = (outputmatlab *) omt_;
     omt->residual = (double *) malloc(*sizer*sizeof(double));
     omt->size_residual = *sizer;
...
void setoutputmatlabpen_(void *omt_, double *pen, int *sizepen){
     outputmatlab *omt;
     int i;

     omt = (outputmatlab *) omt_;
     omt->penalty = (double *) malloc(*sizepen*sizeof(double));
     omt->size_penalty = *sizepen;
...
int create_expetiment_struct(char *file, experiment_total *exptotal, int NPROC, int id,  char *path, int init){
    ...
    exptotal->execution.file = (char *) calloc(10000,sizeof(char));
    exptotal->execution.family_slave = 0;
	strncpy(exptotal->execution.file, file, 10000);
    exptotal->test.output_graph = (char *) calloc(10000, sizeof(char));
	strncpy(exptotal->test.output_graph, path, 10000);
	exptotal->test.output_graph[999] = '\0'; 

    exptotal->test.output_path = (char *) calloc(10000, sizeof(char));
    ...
    if (id == 0) {
      ...
      if (init==1) {
        while (error == -1) {
			strncpy(exptotal->test.output_path, exptotal->test.output_graph, 10000);
...
#ifdef MPI2
    MPI_Bcast(exptotal->test.output_graph, 10000, MPI_CHAR, 0, MPI_COMM_WORLD);
#endif
...
int getnumversion(experiment_total *exptotal){
    ...
    else if (strcmp(exptotal->methodScatterSearch->eSSversion, "hyper") == 0) {
        return 5;
    }
    else if (strcmp(exptotal->methodScatterSearch->eSSversion, "saCDE") == 0) {
        return 6;
    }
...
const char* getversioness(int i){
	...
	retchar="hypersearch";
        ...
        retchar="saCDE - SELF-ADAPTED ASYNCHONOUS COOPERATIVE DE";
    }
...
void destroyexp(experiment_total *exp) {
    ...
    if ((*exp).test.hyper_log != NULL) {
        free((*exp).test.hyper_log);
        (*exp).test.hyper_log=NULL;
    }
...
int chargeglobaloptions_( void *exp_, int *dim_ref, int *ndiverse, int *initiate, int *combination, int *regenerate, 
        char* delete_, int *intens, double * tolf, int *diverse_criteria, double * tolx, int *n_stuck) {
        ...
        if (method->goptions != NULL) {
            ...
            *regenerate = method->goptions->regenerate;
            ...
            if (method->goptions->delete1 == NULL ) {
                ...
                sprintf(delete_,"%s", (const char *) method->goptions->delete1);
            }
