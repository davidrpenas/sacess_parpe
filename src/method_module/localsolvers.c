...
//     res = (output_function *) fitnessfunction(x0, (void *) exp);
     res = (output_function *) evalSB_residuals(x0,  (void *) exp);

//printf("res->R:");
//for (i=0;i<res->size_r;i++) printf("%lf ", res->R[i]);
...
void callparpe_(void * exp1_, void *(*fitnessfunction)(double*, void*), double *x0, long *nevals, int *iters, double *init_time, double *max_time ) {
    experiment_total *exp1;
    int i, D, idp, N;
    void *request_recep;
    ...
    N =  exp1->test.bench.dim;
    double fx;
    long evals_ls;

    if  (exp1->test.bench.translation == 1) 
    	detranslation( x0, exp1->execution.transconst, exp1->test.bench.dim );

    setenv("PARPE_OPTION_SOLVE","0",1);    
    setenv("PARPE_LOG_SIMULATIONS","0",1);
    setenv("PARPE_NUM_SIMULATION_TRIALS","1",1);
    setenv("PARPE_NO_DEBUG","1",1);

//    call_parPE(string_h5, output_h5, idp, x0, &fx, N, nevals, *iters, *init_time, *max_time );
    evals_ls = 0;

    fx = DBL_MAX;

    if (exp1->test.amici_sensi == 1)      call_parPE( idp,  &fx, N, &evals_ls, *iters, *init_time, *max_time, x0, string_h5_forward, output_h5 );
    else if (exp1->test.amici_sensi == 2) call_parPE( idp,  &fx, N, &evals_ls, *iters, *init_time, *max_time, x0,  string_h5_adjoint, output_h5 );
    else call_parPE( idp,  &fx, N, &evals_ls, *iters, *init_time, *max_time, x0, string_h5_forward, output_h5 ); 

    if (( evals_ls > 0) || ( fx != DBL_MAX ) || ( fx != INFINITY ) || ( fx == NAN)) {
    	*nevals = *nevals + evals_ls;

    	if  (exp1->test.bench.translation == 1)
    		translation( x0, exp1->execution.transconst, exp1->test.bench.dim );
    }
...
//        res = (output_function *) fitnessfunction(x, (void *) exp);
        res = (output_function *) evalSB_residuals(x,  (void *) exp);
