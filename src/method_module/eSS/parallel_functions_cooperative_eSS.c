...
void asynchinitmasterandwindows_(void *exp1_, int *sizeser ) {
    ...
    SIZE_ADAP= 9;    
     ...
     exp1[0].execution.adaptation_slave_buffer_recv = (double  *) malloc( SIZE_ADAP * sizeof(double) );
     ...
     if (exp1[0].execution.idp == 0) {
         ...
         for (i = 0; i < (exp1[0].execution.NPROC-1); i++) {
	      exp1[0].execution.receptionbuffermaster[i] = (double *) malloc((*sizeser)* sizeof (double));
              exp1[0].execution.adaptation_master_buffer_send[i] = (double *) malloc( SIZE_ADAP * sizeof (double));
...
// SLAVES
       ...
       MPI_Irecv(exp1[0].execution.adaptation_slave_buffer_recv, SIZE_ADAP, MPI_DOUBLE,0, 155,
...
void adaptationsendmaster_(void *exp1_, int *idsend, int *size_dim, int *ncounter, double *balance, int *accept,
			int *gsolver, int *size_de, double *f, double *cr, int *mt_str) {
    experiment_total *exp1;    
    double *buffersend;
    exp1 = (experiment_total *) exp1_;  
    int SIZE_ADAP;
    ...
    buffersend[3] = *accept;
    buffersend[4] = (double) *gsolver;
    buffersend[5] = (double) *size_de;
    buffersend[6] = *f;
    buffersend[7] = *cr;
    buffersend[8] = (double) *mt_str;
    
    MPI_Isend(buffersend, SIZE_ADAP, MPI_DOUBLE, *idsend, 155, 
...
void checkadaptsettings_(void *exp1_, int *size_dim, int *ncounter, double *balance, int *adaptflag, int *accept,
	int *gsolver, int *dim_de, double *f, double *cr, int *mt_str ){ 
    experiment_total *exp1;    
    int flag;
    exp1 = (experiment_total *) exp1_;  
    int SIZE_ADAP;
	...
	*gsolver  = (int)       exp1[0].execution.adaptation_slave_buffer_recv[4]; 
        *dim_de   = (int)       exp1[0].execution.adaptation_slave_buffer_recv[5];
        *f        = (double)    exp1[0].execution.adaptation_slave_buffer_recv[6];
	*cr       = (double)    exp1[0].execution.adaptation_slave_buffer_recv[7];
	*mt_str   = (int)       exp1[0].execution.adaptation_slave_buffer_recv[8];
        *adaptflag = 1;
