...
CONTAINS
    ...
    FUNCTION hyper(exp1, fitnessfunction, results1, maxfunevals, ftarget) RESULT (outresult)
        ...
        INTEGER :: tam
        INTEGER :: SIZE_CHILDSET
        INTEGER, DIMENSION(:), ALLOCATABLE :: index_aux
        ...
        INTEGER(C_INT) :: getopenmpoption, openmp_pos, zz
...
!-------------------------------------------------------------------------------------------------------!
        ...
        CALL CONFIGURE_SLAVES(exp1, problem1, opts1, common_vars%NPROC, opts1%globaloptions%dim_refset, &
...
!-------------------------------------------------------------------------------------------------------!
        ...
        if ((opts1%globaloptions%global_solver(1:3) .EQ. 'eSS') .OR. (common_vars%idp .EQ. 0 )) then
           cputime1 = 0d0 
...
!----------------------------------------------------------------------------------------------------------------------------------        
                ...
                if ((common_vars%iter .NE. 0) ) then
                   ...
                   CALL adapt_slave(exp1,opts1,problem1,fitnessfunction,common_vars,common_vars%nfuneval,xl_log,xu_log, &
                          ...
                          common_vars%ncounter_slave_recv_sol, common_vars%ncounter_slave_send_sol, &
                          common_vars%pending_adaptation,common_vars%lastevals,bks_in_population_flag,0)        
...
!----------------------------------------------------------------------------------------------------------------------------------            
          ...
          if (opts1%globaloptions%global_solver(1:3) .EQ. 'eSS') then
                SIZE_CHILDSET = CEILING(ess_comm%MaxSubSet2)
          else
                SIZE_CHILDSET = REAL(opts1%globaloptions%dim_refset,&
                                        KIND=SELECTED_REAL_KIND(P=PRECISION_D,R=RANGE_D))
          end if
            
          CALL create_childset ( pop%childset, common_vars%nvar, SIZE_CHILDSET, common_vars%nconst )
          counter = 1
...
!----------------------------------------------------------------------------------------------------------------------------------
! COMBINATION DIFFERENT METHODS: DIFFERENTIAL EVOLUTION AND SCATTER SEARCH
!----------------------------------------------------------------------------------------------------------------------------------
        if (opts1%globaloptions%global_solver(1:2) .EQ. 'DE') then
           CALL combine_differential_evolution(exp1,opts1,common_vars,pop,new_comb,xbest)
           ALLOCATE(index_aux(opts1%globaloptions%dim_refset))
           index_aux = (/(zz, zz = 1, opts1%globaloptions%dim_refset)/)
           cputime1 = calctimeMPI(exp1,time%starttime)
           CALL printcombine(exp1, 2, cputime1)
        else if (opts1%globaloptions%global_solver(1:3) .EQ. 'eSS') then
           CALL combine_scatter_search(exp1,opts1,common_vars,ess_comm,pop,new_comb)
           ALLOCATE(index_aux(size(ess_comm%index)))
           index_aux=ess_comm%index
           cputime1 = calctimeMPI(exp1,time%starttime)
           CALL printcombine(exp1, 1, cputime1)
        end if
...
!----------------------------------------------------------------------------------------------------------------------------------
#ifdef OPENMP
          CALL eval_solution(exp1,opts1,common_vars,SIZE_CHILDSET,pop,&
                                        fitnessfunction,problem1,1,counter,new_comb,index_aux) 
#else
          CALL eval_solution(exp1,opts1,common_vars,SIZE_CHILDSET,pop,&
                                        fitnessfunction,problem1,0,counter,new_comb,index_aux)
#endif
          DEALLOCATE(index_aux)
!----------------------------------------------------------------------------------------------------------------------------------
...
!---------------------------------------------------------------------------------------------------------
        ...
        if ((opts1%globaloptions%global_solver(1:2) .EQ. 'DE')  .AND. (common_vars%idp .NE. 0 )) then
           cputime1 = calctimeMPI(exp1,time%starttime)
           CALL printganthyper(exp1,cputime1,2)
        else if ((opts1%globaloptions%global_solver(1:3) .EQ. 'eSS')  .AND. (common_vars%idp .NE. 0 )) then
           cputime1 = calctimeMPI(exp1,time%starttime)
           CALL printganthyper(exp1,cputime1,1)
        end if
