...
!            CALL call_misqp( problem1,exp1,opts1,fitnessfunction, x0, fval, neval, exitflag, local_solver_vars )
       ...
       else if ( opts1%localoptions%solver(1:5) .EQ. 'parPE' ) then
            CALL callparpe(exp1,fitnessfunction,x0,neval,local_solver_vars%evals_per_iter,iter,NPROC, 0)
       end if
          ...
          if (local_solver_var%stage_1 .eq. 1) then           
            ...
            if ((opts1%localoptions%bestx .eq. 1).AND.(local_solver_var%use_bestx .eq. 1)) then
                ...
                if (allocated(local_solver_var%local_solutions)) then
                    ...
                    DIMEN = [common_vars%nvar, 0]  
                end if

                if (allocated(local_solver_var%initial_points)) then
                    DIMEN2  =  shape(local_solver_var%initial_points)
                else
                    DIMEN2 = [common_vars%nvar, 0]  
                end if

                if ((DIMEN(2) + DIMEN2(2)) .GT. 0) then
                        ALLOCATE(local_init(common_vars%nvar, DIMEN(2) + DIMEN2(2)))
                end if
