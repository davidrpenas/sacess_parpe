...
int extract_element_test(xmlDocPtr doc, xmlNodePtr *root, experiment_testbed *test) {
    ...
    if (extract_element_uniq(doc, cur, sensi) == NULL) {
        test->amici_sensi = 2; 
    }  else {          
    	test->amici_sensi = atoi(extract_element_uniq(doc, cur, sensi)); 
    }
...
int extract_element_method_ScatterSearch(xmlDocPtr doc, xmlNodePtr *root, experiment_method_ScatterSearch *method) {
    ...
    if (cur2 != NULL ) {
		...
		( strcmp(method->loptions->solver, "nl2sol.dn2fb")!=0) &&
                ...
                (strcmp(method->loptions->solver, "parPE")!=0) &&
...
int load_configuration_XML(char *docname, experiment_total *exptotal){
    ...
    if (cur != NULL) {
        ...
        if ((strcmp((const char *) value, "ScatterSearch")==0) || (strcmp((const char *) value, "CeSS") == 0) 
                ...
                (strcmp((const char *) value, "eSSm") == 0 ) || (strcmp((const char *) value, "hyper") == 0 ) || 
		(strcmp((const char *) value, "saCDE") == 0 )) {
            ...
            else if (strcmp((const char *) value, "saCDE") == 0){
                exptotal->methodScatterSearch->eSSversion = "saCDE";
                #ifndef MPI2
                perror(error32);
                exit(32);
                #endif
            ...
            else if (strcmp((const char *) value, "hyper") == 0){
                exptotal->methodScatterSearch->eSSversion = "hyper";
                #ifndef MPI2
                perror(error32);
                exit(32);
                #endif
