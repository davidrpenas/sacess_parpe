...
void initprintfile_(void *exp1_, double *best, int *par, int *idp, double *currenttime, long *evals, int *master) {
    ...
    if (exp1[0].test.output == 1) {
        output->oldbest = *best;
        if (*par == 0) {
            exp1[0].test.log_output = (char *) calloc(1000, sizeof(char));
            exp1[0].test.result_output = (char *) calloc(1000, sizeof(char));
			...
			exp1[0].test.output_path_bs = (char *) calloc(1000, sizeof(char));
			sprintf(exp1[0].test.output_path_bs, "%s/best_solution_id%d.csv", exp1[0].test.output_graph,*idp);
			printf("%s\n", exp1[0].test.output_path_bs);
			p8 = fopen( (const char *) exp1[0].test.output_path_bs, "w");
			fprintf(p8, "Time,bestFx,N_evals,Parameters\n");
			fclose(p8);	
			p8 = NULL;
        } else {
            ...
            if (exp1[0].execution.initpath == 1) {
				...
				sprintf(exp1[0].test.output_path_bs, "%s/best_solution_id%d.csv", exp1[0].test.output_graph,*idp);
				printf("%s\n", exp1[0].test.output_path_bs);
				p8 = fopen( (const char *) exp1[0].test.output_path_bs, "a");
				fprintf(p8, "Time,bestFx,N_evals,Parameters\n");
				fclose(p8);	
				p8 = NULL;
                ...
                exp1[0].test.log_output = (char *) calloc(1000, sizeof(char));
                exp1[0].test.log_percentage = (char *) calloc(1000, sizeof(char));
                exp1[0].test.output_gant_log = (char *) calloc(1000, sizeof(char));
                exp1[0].test.hyper_log = (char *) calloc(1000, sizeof(char));
                exp1[0].test.result_output = (char *) calloc(1000, sizeof(char));     
                ...
                sprintf(exp1[0].test.hyper_log,"%s/hyper_gantt_id%d.csv", exp1[0].test.output_graph,*idp);

                if (*master == 1) {
                    if (*idp != 0) {
						char *temp = (char *) calloc(1000, sizeof(char));
						sprintf(temp,   "/convergence_id%d.csv", *idp);
                        strncat (exp1[0].test.output_graph, temp, strlen( exp1[0].test.output_graph ) + strlen(temp) + 1);
						free(temp);
					}
                } else {
                        char *temp = (char *) calloc(1000, sizeof(char));
			...
			p7 = fopen( (const char *) exp1[0].test.hyper_log, "a");
...
void printbestsolutionfile_(void *exp1_, double *best, double *xbest, double *currenttime, int *evals) {
    ...
    if (exp1[0].test.output == 1) {
		...
		fprintf(p3, "Time,bestFx,N_evals,Parameters\n");
		if (*evals==0) fprintf(p3, "%0.15f,%0.15f,-,",*currenttime, *best);
		else  fprintf(p3, "%0.15f,%0.15f,%ld,",*currenttime, *best, (long int) *evals);
...
void printcombine_(void *exp1_, int *option, double *time) {
	...
	if (*option==1) {
        fprintf(p3, "\tID[%d] - COMBINE_M ESS - %lf\n",   exp1[0].execution.idp, *time);
	} else {
        fprintf(p3, "\tID[%d] - COMBINE_M DE  - %lf\n",   exp1[0].execution.idp, *time);	
	}
        fclose(p3);
        p3 = NULL;
    }
...
void printadaptation_(void *exp1_, double *balance, int *dim, int *counter, double *time,
			int *gsolver, int * mutation, int *size_dim_DE, int *de_f, int *de_cr) {
    ...
    if (exp1[0].test.output == 1) {
	if (*gsolver==2) solver="DE";
	else if (*gsolver==1) solver="eSS";
        p3 = fopen((char *) exp1[0].test.log_output, "a");

        fprintf(p3, "\tID[%d] - ADAPTING THREAD -- %s, balance %lf, eSS Refset %d, n2 %d, F %lf, CR %lf, Mt %d - TIME[%lf]\n",
                exp1[0].execution.idp, solver, *balance, *dim, *counter, *de_f, *de_cr, *mutation, *time );
...
void printadaptationmaster_(void *exp1_, int *origin, int *dest, double *balance, int *dim, int *counter, int *accept, int *adap, double *time,
			 int *gsolver, int *size_dim_de, double *de_f, double *de_cr, int *mt_str) {

    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
    const char * solver, *mt;
  
    if (exp1[0].test.output == 1) {

        if (*gsolver == 1) solver="eSS";
	else if (*gsolver == 2) solver="DE";
	
	if (*mt_str ==     1) mt="best/1";     
	else if (*mt_str ==2) mt="best/2";
        else if (*mt_str ==3) mt="rand/2";
	else  mt="-";

        p3 = fopen((char *) exp1[0].test.log_output, "a");
        if (*accept == 1) {
	    if (*adap == 1) {
            fprintf(p3, "\tID[%d] - SEND ADAPTING THREAD %d [RECV>SEND] -- new config: solver %s, balance %lf, Refset %d, n2 %d, size DE %d, F %lf, CR %lf, Mt %s [best id %d] [ACCEPT] - TIME[%lf]\n",
                exp1[0].execution.idp,  *origin, solver, *balance, *dim, *counter, *size_dim_de,*de_f,*de_cr,mt, *dest, *time );
	    }
            if (*adap == 2) {
            fprintf(p3, "\tID[%d] - SEND ADAPTING THREAD %d [MAXEVALSx2] -- new config: solver %s, balance %lf, Refset %d, n2 %d, size DE %d, F %lf, CR %lf, Mt %s [best id %d] [ACCEPT] - TIME[%lf]\n",
                exp1[0].execution.idp,  *origin, solver, *balance, *dim, *counter, *size_dim_de,*de_f,*de_cr,mt, *dest, *time );
            }
            if (*adap == 3) {
            fprintf(p3, "\tID[%d] - SEND ADAPTING THREAD %d [MAXEVALS&SEND=0] -- new config: solver %s, balance %lf, Refset %d, n2 %d, size DE %d, F %lf, CR %lf, Mt %s [best id %d] [ACCEPT] - TIME[%lf]\n",
                exp1[0].execution.idp,  *origin, solver, *balance, *dim, *counter, *size_dim_de,*de_f,*de_cr,mt, *dest, *time );
            }

        } else {
            fprintf(p3, "\tID[%d] - SEND ADAPTING THREAD %d  -- new config: solver %s, balance %lf, Refset %d, n2 %d, size DE %d, F %lf, CR %lf, Mt %s [best id %d] [REJECT] - TIME[%lf]\n",
                exp1[0].execution.idp,  *origin, solver, *balance, *dim, *counter, *size_dim_de,*de_f,*de_cr,mt, *dest, *time );            
        }
        fclose(p3);
        p3 = NULL;
...
void printdesadaptationmaster_(void *exp1_, int *origin, int *dest, double *balance, int *dim, int *counter, double *time,
    ...
    if (exp1[0].test.output == 1) {
        ...
        fprintf(p3, "\tID[%d] - SEND (DES)ADAPTING THREAD %d -- new config: solver %s, balance %lf, Refset %d, n2 %d, size DE %d, F %lf, CR %lf, Mt %s [best id %d] - TIME[%lf]\n", exp1[0].execution.idp,  *origin, solver, *balance, *dim, *counter, *size_dim_de,*de_f,*de_cr,mt, *dest, *time );
...
void printmasterocurrencesend_(void *exp1_, int *vector, int  *size, int *masterslave, int *dim_refset, int *n2, double *balance ) {
    ...
    sizelocal=1;
    cooperativegathertelementint_(exp1, (double *) dim_refset, &sizelocal,(double *)  dim_refset_vector);
    cooperativegathertelementint_(exp1, (double *) n2, &sizelocal, (double *) n2_vector);
...
void printputcheckmasterlog_(void *exp1_, double *new_, double *old ) {
    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
...
void printcomparenewsolutionslavelog_(void *exp1_, double *new_, double *old, double *dist, double *time, double *th ) {
    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
    ...
    if (exp1[0].test.output == 1) {
		...
		exp1[0].execution.idp, *new_, *old, *dist, *th, *time  );
        fclose(p3);
        p3 = NULL;
    }
...
void printreceptioncompare_(void *exp1_, double *new_, double *old, double *dist, double *time, double *maxthreshold ) {
    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
    ...
    if (exp1[0].test.output == 1) {
		...
		exp1[0].execution.idp, *new_, *old, *dist, *maxthreshold, *time  );
        fclose(p3);
        p3 = NULL;
    }
...
void printcomparenewsolutionmasterlog_(void *exp1_, double *new_, double *old, double *dist, double *time, double *maxthreshold ) {
    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
...
void printfinalsendslavelog_(void *exp1_, double *new_, double *time ) {
    experiment_total *exp1;
    exp1 = (experiment_total *) exp1_;
    FILE *p3;
    ...
    if (exp1[0].test.output == 1) {
		...
		exp1[0].execution.idp, *new_, *time  );
        fclose(p3);
        p3 = NULL;
    }
 ...
 */
void printiterationcess_(void *exp1_, double *best, long *evaluation_local, double *currenttime,int *locals, int *migration) {
...
void matlab_plot_file_hyper_gant(experiment_total exp1,  int init1, int end, char* stringpath, int maxsize) {
        ...
        mfile = ".m";
        path = (char *) calloc(1000,sizeof(char));
        memcpy(path,exp1.test.hyper_log,strlen(exp1.test.hyper_log)-4-4);
        strcat( path, mfile);
        p4 = fopen(path, "a");
        if (p4 == NULL) exit(0);
        ...
        while (salir == 0) {
            ...
            if ((time != -1 && state != -1)) {
                if (init == 0) {
                    if ((init1 == 1)&&((state==1)||(state==2))) {
                        fprintf(p4,"clear all;\n");
                        fprintf(p4,"close all;\n\n");
                        fprintf(p4,"data = [ %lf ", time);
                        contador++;                        
                    }
		    else if ((init1==1)&&(state!=1)&&(state!=2)) {
                        fprintf(p4,"clear all;\n");
                        fprintf(p4,"close all;\n\n");
                        fprintf(p4,"data = [ ");
                        contador++;
        ...
        if (end == 1) {
            ...
            fprintf(p4, "figure\n");            
            fprintf(p4, "h1 = barh(slots,'stacked');\n");
            fprintf(p4, "hold on;\n");
...
void matlab_plot_file_hyper_gant_adapt(experiment_total exp1,  int init1, int end, char* stringpath, int maxsize) {
        ...
        while (salir == 0) {
            ...
            if ((time != -1 && state != -1)) {
                if ((init == 0)) {
                    if ((init1 == 1) && (state == 3)) {
                        fprintf(p4,"adapt = [ %lf ", time);
                        contador++;                        
                    }
		    else if ((init1 == 1) && (state != 3)) {
			fprintf(p4,"adapt = [ 0", time);
			contador++;
		    }
		    else {
			if (state == 3) {
        ...
        if (end == 1) {
            ...
            fprintf(p4, "adapty = [ ];\n");
            fprintf(p4, "adaptx = [ ];\n");
            fprintf(p4, "slots2 = adapt(:,1:end);\n");
            
            fprintf(p4, "for i =1:length(slots2(1,:))\n");
            fprintf(p4, "    for j = 1:1:length(slots2(:,1))\n");
            fprintf(p4, "        if slots2(j,i) > 0\n");
            fprintf(p4, "           adaptx = [slots2(j,i) adaptx];\n");
            fprintf(p4, "           adapty = [j adapty];\n");
            fprintf(p4, "        end\n");   
            fprintf(p4, "    end\n");               
            fprintf(p4, "end\n\n");

            fprintf(p4, "\n");            
            fprintf(p4, "h2 = plot(adaptx,adapty,'rs','MarkerSize',10,'MarkerFaceColor','b','MarkerEdgeColor', [0.5 0 0]);\n");
...
void matlab_plot_file_hyper_gant_final(experiment_total exp1) {
	...
	fprintf(p4, "h = [h2 h1];\n");
	fprintf(p4, "title('Visual Logs: hyperheuristic');\n");
	fprintf(p4, "xlabel('Time','FontSize',25);\n");
	fprintf(p4, "ylabel('Processes','FontSize',25);\n");
	fprintf(p4, "set(gca, 'YDir', 'reverse','FontSize',25);\n");
	fprintf(p4, "for i = 2:2:(length(slots(1,:)))\n");
	fprintf(p4, "    set(h(i), 'facecolor', 'red', 'EdgeColor', 'red');\n");
	fprintf(p4, "    if ( (i+1) < length(slots(1,:)))\n");
	fprintf(p4, "       set(h(i+1), 'facecolor', 'green', 'EdgeColor', 'green');\n");
	fprintf(p4, "    end \n");
	fprintf(p4, "end\n");
	fprintf(p4, "legend(h,'Adaptations','DE','eSS');\n");
	fprintf(p4, "set(gcf,'PaperPosition',[0 0 80 60]);\n");
...
//                remove(string6);
 ...
 //               remove(string4);
        ...
        string3 = (char *) malloc(100*sizeof(char));
        vectstring2 = (char *) malloc(100*NPROC*sizeof(char));
        sprintf(string3,"%s",exp1.test.hyper_log);
        string4 = (char *) malloc(100*sizeof(char));
        ...
        if (idp == 0 ) {
            ...
            for (i=0;i<NPROC;i++) {
                ...
                if (i==0) init=1; else init=0;
                if (i == (NPROC-1)) end=1; else end=0;
                matlab_plot_file_hyper_gant(exp1, init, end, string4, maxsize); 
            }
	    // PUNTOS
            for (i=0;i<NPROC;i++) {
                memmove(string4, &vectstring2[i*100], 100* sizeof(char));
                if (i==0) init=1; else init=0;
                if (i == (NPROC-1)) end=1; else end=0;
                matlab_plot_file_hyper_gant_adapt(exp1, init, end, string4, maxsize);
            }
	    // FINAL
	    matlab_plot_file_hyper_gant_final(exp1);
...
void graphs_message(experiment_total *exp) {
    ...
    if ((idsolver == 3) || (idsolver == 5) || (idsolver == 6)) {
