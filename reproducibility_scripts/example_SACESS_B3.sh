...
# have to be equal to 1 (only one thread per MPI processor).
  ...
  export HOME=/home/davidp/hypersearch5
  NPROC=11
...
#  mpirun -np  $NSLOTS $HOME/hypersearch/bin/paralleltestbed1  $1 $2
mpirun -np $NPROC $HOME/bin/paralleltestbed1 $HOME/inputs/TEMPLATE_B3_SACESS.xml $HOME/output/test_b3_saCeSS
